...
class Contests extends CI_Controller {
	...
	//$this->load->model('submission_model') or die("<h1>Error </h1>");
...
//	print_r($data['contests_short_list']); 
...
//	echo " </h1>";
	...
	$this->load->model("sub_model");
        ...
        {
echo "<h1>".$contest_id."</h1>";
//	$this->sub_model->no_of_submissions(1,1,1,1);	

           // $this->load->model('sub_model')or die("<h1>Error</h1>");
          $contest_data['users'] = $this->sub_model->no_of_users($contest_id);
	//	echo "<h1> $contest_data['users'] </h1>";	

	
             $contest_data['accepted_sub'] = $this->sub_model->no_of_submissions('%','%',$contest_id,111);
             $contest_data['total_sub'] = $this->sub_model->no_of_submissions('%','%',$contest_id,'%');
                ...
                if($contest_data['contest_details'][0]->start_time < $time || $this->session->userdata('username')=="admin")
                {
